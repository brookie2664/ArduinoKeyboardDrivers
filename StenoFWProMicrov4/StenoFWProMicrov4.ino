...
boolean currentstroke[ROWS][COLS];
...
boolean qwertykeyboardtoggle = false;
...
#define NKRO 2
int protocol = NKRO;
//plovertoggle is initialized as true.
...
void fn1fn2() {
  ...
  protocol = NKRO; // is this how you finish with an else statement? 
  }
}
...
void togglestenokeyboard() {
  ...
  else if (plovertoggle == false && qwertykeyboardtoggle == false){
    plovertoggle = false;
    analogWrite(stenoledPin, 0);
    return;
  }
...
void sendplovertogglechord() {
  //Keyboard.press(); // add whatever I define as plover's toggle chord into this. 
  //delay(20);
  //Keyboard.releaseAll();
}
...
void normalkeyboard() {
  ...
  // 
char keyboardmapping1[ROWS][COLS] = {
  {179, '\'', ',', '.', 'p', 'y'}, // {'f', 'g', 'c', 'r', 'l', ' '},
  {193, 'a', 'o', 'e', 'u', 'i'}, //  {'d', 'h', 't', 'n', 's', ' '},
  {129, ';', 'q', 'j', 'k', 'x'}, //  {'b', 'm', 'w', 'v', 'z', ' '},
  {128, 131, 130, 177, ' ',  32}, //  {' ', ' ', ' ', ' ', ' ', ' '}  // l_control, win, alt, esc, 'raise', space
  {'f', 'g', 'c', 'r', 'l',   8}, // ascii table says 8, leonardo table says 178 (backspace)
  {'d', 'h', 't', 'n', 's', '-'}, // to get enter, there needs to be a layer raise. Enter.
  {'b', 'm', 'w', 'v', 'z', 133},
  { 32, ' ', 216, 217, 218, 215}  // space, 'lower', left, down, up, right
};
  /*int keyCounter = 0;
  char qwertyKeys[ROWS * COLS];
  boolean firstKeyPressed = false;
...
boolean recordCurrentKeys2() {
  ...
  for (int i = 0; i < ROWS; i++) {
    for (int j = 0; j < COLS; j++) {
      if (keyboardkeyreadings[i][j] == true) { // can I get rid of the any key pressed thing?
        currentstroke[i][j] = true;
        isAnyKeyPressed = true;
      }
    }
  }
...
void checkAlreadyDebouncingKeys2() {
  for (int i = 0; i < ROWS; i++) {
    for (int j = 0; j < COLS; j++) {
      if (debouncingKeys2[i][j] == true && keyboardkeyreadings[i][j] == false) { 
        debouncingKeys2[i][j] = false; //I think I can use this to trigger keyboard.release.
        //Keyboard.release(keyboardoutput[i]) //this is qwertykeys[i] from above.
        continue;
      }
      if (debouncingKeys2[i][j] == true && micros() - debouncingMicros2[i][j] / 1000 > debounceMillis) {
        isStrokeInProgress = true; // need to see if i can get rid of this
        currentstroke[i][j] = true;
        return;
      }
    }
